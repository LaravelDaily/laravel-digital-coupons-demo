...
@can('code_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.codes.create') }}">
                ...
                <tbody>
                    @foreach($codes as $key => $code)
                        <tr data-entry-id="{{ $code->id }}">
                            ...
                            <td>
                                @can('code_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.codes.show', $code->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('code_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.codes.edit', $code->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('code_delete')
                                    <form action="{{ route('admin.codes.destroy', $code->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('code_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.codes.massDestroy') }}",
