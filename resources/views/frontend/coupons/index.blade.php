                            ...
                            <tbody>
                                @foreach($coupons as $key => $coupon)
                                    <tr data-entry-id="{{ $coupon->id }}">
                                        ...
                                        <td>
                                            @can('coupon_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.coupons.show', $coupon->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('coupon_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.coupons.edit', $coupon->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('coupon_delete')
                                                <form action="{{ route('frontend.coupons.destroy', $coupon->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('coupon_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.coupons.massDestroy') }}",
